...
var Chaincode = class {
  ...
  async confirmTimestamp(stub, args) {
    if (args.length != 1) {
      throw new Error(
        "Incorrect number of arguments. Expecting assetID as an argument"
      );
    }
  ...
  async updateAssetOwnership(stub, args) {
    ...
    if (args.length != 2) {
      throw new Error(
        "Incorrect number of arguments. Expecting 2, but received" + args.length
      );
    }
    ...
    try {
      console.info(
        "updatedAssetInfo.assetID   ===== : ",
        updatedAssetInfo.assetID
      );
      console.info(
        "updatedAssetInfo.owner     ===== : ",
        updatedAssetInfo.owner
      );
      console.info(
        "updatedAssetInfo.createdAt ===== : ",
        updatedAssetInfo.createdAt
      );
      console.info(
        "updatedAssetInfo.expiredAt ===== : ",
        updatedAssetInfo.expiredAt
      );
    } catch (err) {
      console.info(err);
    ...
    await stub.putState(
      searchKey,
      Buffer.from(JSON.stringify(updatedAssetInfo))
    );
  }
